// ============================================================================
// tb_interface.sv
// ECE 410/510 — Testbench: conv_interface
//
...
//   iverilog -g2012 -o sim_if.out interface.sv tb_interface.sv && vvp sim_if.out
// ============================================================================
...
module tb_interface;
    ...
    // Presents AW and W simultaneously (standard AXI4-Lite master practice).
    // Waits for AWREADY & WREADY, then waits for BVALID and accepts BRESP.
    task axil_write;
        input [3:0]  addr;
        input [31:0] data;
        ...
        // spin until both channels are accepted (ready signals go high)
        while (!(awready && wready)) @(posedge clk);
        // posedge where DUT samples the handshake
        @(posedge clk); #0.1;
    ...
    // ── AXI4-Lite read task ───────────────────────────────────────────────
    // Presents AR, waits for ARREADY, then waits for RVALID and captures data.
    task axil_read;
        input  [3:0]  addr;
        output [31:0] rd_data;
        @(posedge clk); #0.1;
    ...
    initial begin : stim
        ...
        // synchronous reset for 4 cycles
        ...
        //   Full handshake: AW channel → W channel → B response
        // ════════════════════════════════════════════════════════════════
        axil_write(4'h8, 32'h00000009);

        // bresp is a constant OKAY (2'b00); bvalid was seen inside the task
        ...
        //   Verifies internal register holds what was written in T1
        // ════════════════════════════════════════════════════════════════
        ...
        //   TREADY must be high whenever TVALID is asserted (always-ready).
        //   Transfer is considered complete when TVALID & TREADY are both 1.
        // ════════════════════════════════════════════════════════════════
        @(posedge clk); #0.1;
        tdata  = {{480{1'b0}}, 32'hDEAD_BEEF};  // non-trivial 512-bit pattern
        tvalid = 1'b1;
        tlast  = 1'b1;
        ...
        // ════════════════════════════════════════════════════════════════
        // TEST 4 — CTRL start pulse
        //   Write 0x1 to CTRL (0x00); DUT must assert core_start for one cycle.
        //   Sample core_start at 0.1 ns after the handshake posedge.
        // ════════════════════════════════════════════════════════════════
        @(posedge clk); #0.1;
        ...
        while (!(awready && wready)) @(posedge clk);
        @(posedge clk); #0.1;           // handshake posedge — reg_start latched
        awvalid  = 1'b0;  wvalid = 1'b0;
        got_start = core_start;         // sample core_start right after posedge
